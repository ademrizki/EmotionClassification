    ...
    try:
        while True:
            ...
            for face_coordinates in faces:
                ...
                draw_text(face_coordinates, rgb_image, emotion_mode, color, 0, -10, 5, 10)
            ...
            if elapsed > 316:
                try:
                    ...
                    c.drawString(100,300,"Total Persentasi :    "+str(per_total))
                    if (per_total == 0):
                        c.drawString(100,250,"Tidak Menangkap Emosi")
                    c.save()
                    #os.system("Lap_"+npm+".pdf")
                    video_capture.release()
                    out.release()
...
def faceStim():
    ...
    cv2.moveWindow("window_frame", 20,20)
    fourcc = cv2.VideoWriter_fourcc(*'MJPG')
    out = cv2.VideoWriter('emo_'+str(npm.get())+'.mp4', fourcc, 20.0, (1280,720))
    video_capture = cv2.VideoCapture(0)
    start_time = time.time()
    try:
        while True:
...
def tutorial():
    threading.Thread(target=faceTut).start()
    threading.Thread(target=stimulus).start()
    
def eksp():
    threading.Thread(target=faceStim).start()
    threading.Thread(target=stimuli).start()
...
btn1_stimulus = tkinter.Button(root, text="Tutorial", command=tutorial, font=("Verdana", 15))
btn2_stop = tkinter.Button(root, text="Stimulus", command=eksp, font=("Verdana", 15))
#btn3_EnS = tkinter.Button(root, text="Emosi dengan Stimulus", command=twoVid, font=("Verdana", 20))
btn4_ekspresi = tkinter.Button(root, text="Stop", command=exit, font=("Verdana", 15))
...
btn1_stimulus.grid(row=4, column = 0, columnspan=5, sticky = 'W', pady=10, ipadx=10, padx=10)
btn2_stop.grid(row=4, column = 1, columnspan=5, sticky = 'E', pady=10, ipadx=10, padx=10)
#btn3_EnS.grid(row=4, column = 2)
btn4_ekspresi.grid(row=5, column = 1, columnspan=5, pady=10, ipadx=10)
